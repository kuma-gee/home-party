...
- **Ecto-Vacuum** – Capture tool. Sucks in a fully visible ghost over ~3 seconds. While actively sucking, the Hunter moves slowly. If another ghost haunts the vacuum, the captured ghost escapes and the vacuum is stunned for 5 seconds.
...
- **Single top-down view** of the haunted house on a shared TV/monitor.
  - Each ghost appears as a colored ghosts. The Hunter appears on the shared-screen map only when within ~10 meters of any ghost. The Hunter disappears when he moves out of range. This gives ghosts directional warning that the Hunter is near, without perfect omniscience.
...
- **Abilities** (no cooldowns — usage is limited by context and opportunity):
  ...
  - **Haunt** – Flicker lights, creak doors, or possess an object (doll, mannequin, furniture). While haunting, the ghost is hidden inside the object. The object may twitch or emit subtle audio. Haunting the Ecto-Vacuum triggers a rescue: the captured ghost escapes and the vacuum is stunned for 5 seconds.
...
- **Control Scheme** – Phone touch controls (joystick + ability buttons). The phone screen shows no game data — it acts only as a controller. All info (map, task markers, ghost positions) is displayed on the shared desktop screen.
...
- **Capture sequence:**
  ...
  4. Any other ghost can **Haunt the vacuum** → the captured ghost escapes, the vacuum is stunned for 3 seconds (cannot shoot).
  6. The Hunter can **Flash a haunted vacuum** to purge the haunting ghost, forcing them out and resetting the rescue attempt.
...
- Pass VR headset to the next player. Rotate screen/game-master duty.
