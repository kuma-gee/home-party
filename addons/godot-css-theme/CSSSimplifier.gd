...
func simplify(stylesheet: Stylesheet) -> Stylesheet:
	var values = {BASE_GROUP: {}}
	var base_props_by_class = {}

	for cls in stylesheet.get_classes(BASE_GROUP):
		var new_props = _simplify_class(stylesheet, cls, BASE_GROUP)
		base_props_by_class[cls] = new_props
		values[BASE_GROUP][cls] = {Stylesheet.DEFAULT_STATE: new_props}

	for class_group in stylesheet.get_class_groups():
		if class_group == BASE_GROUP:
			continue

		values[class_group] = {}

		for cls in stylesheet.get_classes(class_group):
			var new_props = _simplify_class(stylesheet, cls, class_group)

			var merged = base_props_by_class.get(cls, {}).duplicate(true)
			merged.merge(new_props, true)

			values[class_group][cls] = {Stylesheet.DEFAULT_STATE: merged}

	return Stylesheet.new(values, stylesheet.get_css_file())


func _simplify_class(stylesheet: Stylesheet, cls: String, class_group: String) -> Dictionary:
	var default_props = stylesheet.get_class_properties(cls, class_group, Stylesheet.DEFAULT_STATE).duplicate(
		true
	)
	var new_props = default_props.duplicate(true)

	for state in stylesheet.get_class_states(cls, class_group):
		var props = default_props.duplicate(true)
		props.merge(stylesheet.get_class_properties(cls, class_group, state).duplicate(true), true)
