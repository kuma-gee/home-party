...
func _validate_text(text: String) -> bool:
    # Code128A 支持所有ASCII字符 (0-95) 和控制字符
    for i in range(text.length()):
        var char_code := text.unicode_at(i)
        if char_code > 95 and char_code < 128:
            # 小写字母不在Code Set A中
            return false
        if char_code > 127:
            return false
...
# 将字符转换为Code Set A的值
# Code Set A: ASCII 0-95 (控制字符 + 大写字母 + 数字 + 符号)
func _char_to_value_a(char_code: int) -> int:
    if char_code >= 0 and char_code <= 31:
        # 控制字符 (0-31) 映射到值 64-95
        return char_code + 64
    elif char_code >= 32 and char_code <= 95:
        # 可打印字符 (32-95) 映射到值 0-63
        return char_code - 32
    return -1
