...
func _generate_bars() -> void:
    ...
    # 使用Start C
    var start_code := START_C
    values.append(start_code)
    
    # 编码文本 (每两个数字作为一个值，00-99)
    for i in range(0, barcode_text.length(), 2):
        var pair := barcode_text.substr(i, 2)
        var value := int(pair)
