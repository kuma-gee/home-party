...
export interface InputLayoutMessage extends Message {
  ...
  layout: "joystick" | "buttons" | "skill_select";
}
...
export class WebSocketClient {
  ...
  onInputLayoutReceived?: (layout: "joystick" | "buttons" | "skill_select") => void;
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.ws.onopen = () => {
          ...
          this.onReconnecting?.(false, 0, this.maxReconnectAttempts);
          this.onConnected?.();
          resolve();
        };
  ...
  private async messageToString(event: MessageEvent<any>): Promise<string> {
    let messageText = '';
  ...
  private handleMessage(message: Message) {
    switch (message.msg) {
      ...
      case MessageType.GameClientSession:
        ...
        this.webrtcClient
          ?.setRemoteDescription(sessionMsg.type, sessionMsg.sdp)
          .catch((error) =>
      ...
      case MessageType.GameClientIceCandidate:
        ...
        this.webrtcClient
          ?.addIceCandidate(iceMsg.mid, iceMsg.index, iceMsg.sdp)
          .catch((error) =>
      ...
      case MessageType.InputLayout:
        ...
        if (inputLayoutMsg.layout === "joystick" || inputLayoutMsg.layout === "buttons" || inputLayoutMsg.layout === "skill_select") {
